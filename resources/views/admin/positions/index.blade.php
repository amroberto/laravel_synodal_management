...
@section('content_header')
    <h1 class="m-0 text-dark">{{ __('Cargos') }}</h1>
@stop
...
<div class="container-fluid">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{ __('Cargos') }}</li>
        </ol>
    </nav>
    <!-- Alertas -->
    @if (session('success'))
    ...
    <x-adminlte-card>
        <div class="mb-3">
            <a href="{{ route('admin.positions.create') }}" class="btn btn-primary">
                <i class="fas fa-plus"></i> {{ __('Adicionar Novo Cargo') }}
            </a>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                                ...
                                <form action="{{ route('admin.positions.destroy', $position) }}" method="POST" class="d-inline"
                                      onsubmit="return confirm('{{ __('Tem certeza que deseja excluir este cargo? Esta ação não pode ser desfeita.') }}');">
                                    @csrf
                                    @method('DELETE')
